...
---
# Getting Started
Before all of this, to use `sgr.ps1`, make sure you set your `ExecutionPolicy` to `RemoteSigned`.
## Installation
1. Download `sgr.ps1`. That's it
- ***Optionally*** add `sgr` to your PATH
- ***Optionally*** create `dirs.txt` in the same directory as `sgr` and put directories containing git repos.
    - if there's no `dirs.txt` in the same directory as `sgr`, it will create one containing only one line:
      ```
...
```
{
    ...
    "command": {
        "action": "newTab",
        "commandline": "C:\\Program Files\\PowerShell\\7\\pwsh.exe -noexit -c \"sgr\""
    }
}
```
...
```
{
    ...
    "command": {
        "action": "newTab",
        "commandline": "C:\\Program Files\\PowerShell\\7\\pwsh.exe -noexit -c \"sgr && nvim .\""
    }
}
```
